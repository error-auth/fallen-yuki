...
buttons = [
    ...
    [
        InlineKeyboardButton(text=" sᴜᴩᴩᴏʀᴛ ɢᴄ ", url=f"https://t.me/{SUPPORT_CHAT}"),
    ],
    [
        InlineKeyboardButton(text=" ᴏᴡɴᴇʀ ", url=f"https://t.me/ghost_kun"),
...
➲ sᴛᴀʀᴛ ᴍᴇ ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴄʜᴀᴛ ʙʏ sᴇɴᴅɪɴɢ /start"""
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            ...
            update.effective_message.reply_sticker(
                "CAACAgUAAx0CfWI6eQACJZpmqywfVi2QD64nWXQ9ZgrV9pyvrQACfQ8AAmPhGVZDxgIQDMk9tDUE"
            )
...
def Fallen_about_callback(update: Update, context: CallbackContext):
    ...
    if query.data == "fallen_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
            text=f"*sᴜᴘ,  *I'ᴍ {BOT_NAME}*"
            "\n*ᴀ sɪᴍᴘʟᴇ ᴀɴɪᴍᴇ ʙᴀsᴇᴅ ᴘᴏᴡᴇʀғᴜʟ ʙᴏᴛ*"
            "\n\n╭───────────────────"
            f"\n*➻ ᴜᴩᴛɪᴍᴇ »* {uptime}"
            f"\n*➻ ᴜsᴇʀs »* {sql.num_users()}"
            f"\n*➻ ᴄʜᴀᴛs »* {sql.num_chats()}"
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text=" sᴜᴩᴩᴏʀᴛ ɢᴄ ", url=f"https://t.me/{SUPPORT_CHAT}"
                        ),
                        InlineKeyboardButton(
...
def settings_button(update: Update, context: CallbackContext):
    ...
    try:
        if mod_match:
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(
                "Sup! Here are quite few settings for {} - go ahead "
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(
                text="Sup! Here are quite few settings for {} - go ahead "
